...
; *** Определение кучи ***
; Определить переменную размер кучи Heap_Size как 512 байт (0x200)
Heap_Size      EQU     0x200
; Объявить секцию памяти для размещения кучи без инициализации памяти с атрибутом выравнивания по 8 байт
                AREA    HEAP, NOINIT, READWRITE, ALIGN=3
; Метка начала кучи
__heap_base
; Зарезервировать область памяти под кучу размеров с Heap_Size
Heap_Mem        SPACE   Heap_Size
; Метка конца кучи
__heap_limit

                PRESERVE8   ; Выравнивание
                THUMB       ; Система команд
...
; Инициализация процессора через внешнюю процедуру SystemInit, с последующим возвратом и передачей управления в пользовательскую программу
        IMPORT  SystemInit
        IMPORT  __main
; Передать управление SystemInit (внешняя метка) с предварительным сохранением адреса возврата в Link регистр                
                 LDR     R0, =SystemInit    ; Псевдокоманда: загрузить адрес SystemInit в регистр R0
                 BLX     R0                 ; Перейти по адресу в R0 с сохранением адреса возврата
                 LDR     R0, =__main        ; Псевдокоманда: загрузить адрес __main в регистр R0
...
FPU_IRQHandler  
                ...
                ALIGN   ; Выровнить текущую кодовую секцию по 32-рахрядному слову
...
__user_initial_stackheap
                 ...
                 END    ; конца ассемблерного текста.
